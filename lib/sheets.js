...
const { loadStaff, loadRegistry, saveRegistry, getSpreadsheetIdForYear, getRecordStatusForDate } = require('./data');
...
function buildSheetHeaderRow(staffList) {
  // 列を持つスタッフのみ対象。office/admin 等は列を持たないため除外する
  // （colToIdx(undefined) でクラッシュし、新年シート作成が失敗するのを防ぐ）。
  const columned = staffList.filter(s => s.type === 'nurse' ? (s.kaigo_col && s.iryo_col) : s.col);
  if (columned.length === 0) return ['日付', '曜日'];
  const maxIdx = Math.max(
    ...columned.map(s => s.type === 'nurse' ? colToIdx(s.iryo_col) : colToIdx(s.col))
  );
  const row = new Array(maxIdx + 1).fill('');
  row[0] = '日付'; row[1] = '曜日';
  for (const s of columned) {
    if (s.type === 'nurse') {
...
async function hasRecordForDate(staff, dateStr) {
  // 列定義がundefinedのスタッフは記録ありとして扱い、リマインダーを出さない
...
// 入力状況は SQLite 正本から判定する（Sheetは読まない）。段階4b。
...
/**
 * 複数rangeの値を一括取得する（batchGet）
 * @returns {Promise<Array>} valueRanges 配列
 */
async function batchGetValues(spreadsheetId, ranges) {
  const api = await getSheets();
  const resp = await sheetsRetry(() =>
    api.spreadsheets.values.batchGet({ spreadsheetId, ranges })
  );
