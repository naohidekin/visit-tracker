...
function createSchema(db) {
  db.exec(`
    ...
    -- 訪問件数（SQLite正本化の移行用。当面は Google Sheets と二重書き込み）
    -- 看護師: kaigo(介護)+iryo(医療) / PT: value（他方はNULL）
    ...
    -- インデックス
    CREATE INDEX IF NOT EXISTS idx_notice_read_staff ON notice_read_status(staff_id);
    CREATE INDEX IF NOT EXISTS idx_attendance_staff  ON attendance(staff_id);
    CREATE INDEX IF NOT EXISTS idx_reminders_staff   ON reminders_sent(staff_id);
    CREATE INDEX IF NOT EXISTS idx_reset_tokens_staff ON reset_tokens(staff_id);
    CREATE INDEX IF NOT EXISTS idx_webauthn_staff    ON webauthn_credentials(staff_id);
    CREATE INDEX IF NOT EXISTS idx_standby_staff     ON standby_records(staff_id);
  `);

  // STEP 7: staff テーブル列正規化（冪等: 列が既存ならスキップ）
  const staffCols = ['is_admin INTEGER DEFAULT 0', 'archived INTEGER DEFAULT 0', 'type TEXT', 'hire_date TEXT'];
  for (const col of staffCols) {
    try { db.exec(`ALTER TABLE staff ADD COLUMN ${col}`); } catch {}
  }
  // 既存JSONデータから新列にコピー（未コピー行のみ）
  db.exec(`
    UPDATE staff SET
      is_admin  = COALESCE(json_extract(data, '$.is_admin'), 0),
      archived  = COALESCE(json_extract(data, '$.archived'), 0),
      type      = json_extract(data, '$.type'),
      hire_date = json_extract(data, '$.hire_date')
    WHERE type IS NULL
  `);
}
