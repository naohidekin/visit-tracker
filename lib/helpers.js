...
const { STANDBY_FEES } = require('./constants');
...
function getStandbyFee(dateStr) {
  ...
  const d = new Date(dateStr + 'T00:00:00');
  const dow = d.getDay();
  if (holidayJp.isHoliday(d) || customHols.has(dateStr)) return { fee: STANDBY_FEES.holiday,  category: '祝日' };
  if (dow === 0) return { fee: STANDBY_FEES.sunday,   category: '日曜' };
  if (dow === 6) return { fee: STANDBY_FEES.saturday, category: '土曜' };
  return { fee: STANDBY_FEES.weekday, category: '平日' };
}
...
// 指定範囲の営業日数を算出（スタッフ個人の有給・欠勤を考慮しない）
function countBusinessDaysRange(startDate, endDate) {
  let count = 0;
...
function countBusinessDays(year, month) {
  ...
  return countBusinessDaysRange(startDate, endDate);
}

// 出勤確定 + 有給データから指定範囲の出勤予定日数を算出
function getExpectedWorkingDaysRange(staffId, startDate, endDate) {
...
function getExpectedWorkingDays(staffId, year, month) {
  ...
  const startDate = `${year}-${String(month).padStart(2,'0')}-01`;
  const endDate = `${year}-${String(month).padStart(2,'0')}-${String(daysInMonth).padStart(2,'0')}`;
  return getExpectedWorkingDaysRange(staffId, startDate, endDate);
}
...
module.exports = {
  ...
  isOnLeaveToday,
  countBusinessDays,
  countBusinessDaysRange,
  getExpectedWorkingDays,
  getExpectedWorkingDaysRange,
};
