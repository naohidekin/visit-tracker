...
// 有給付与の時期が到来しているが、まだ付与日数（leave_granted）に反映されていない
// スタッフの情報を返す。反映済み・対象外（入社日未設定・アーカイブ・初回付与未到達）なら null。
// 判定: 付与規定上あるべき付与日数 (calcLeaveGrantDays) > 現在の付与日数 (leave_granted)
// → 管理者が付与日数を更新すればアラートは自動的に解消される（ステートレス判定）
function calcPendingGrant(staff, todayStr) {
  ...
  const dueDays = calcLeaveGrantDays(staff.hire_date, today);
  if (dueDays <= 0) return null;               // まだ初回付与（6ヶ月）に未到達
  const granted = staff.leave_granted || 0;
  if (granted >= dueDays) return null;         // 既に反映済み

  // 到達済みの最新マイルストーン（付与基準日）を特定
  const hire = new Date(staff.hire_date);
  const now  = new Date(today);
  let reached = null;
  for (const t of LEAVE_GRANT_TABLE) {
    if (hasReachedGrantDate(hire, now, t.months)) reached = t;
  }
  if (!reached) return null; // dueDays>0 なら通常ここには来ない（保険）

  return {
    grant_days: dueDays,
    granted,
    reached_months: reached.months,
    reached_date: toDateStr(addMonthsToDate(hire, reached.months)),
    tenure_label: formatTenureLabel(reached.months),
  };
}
