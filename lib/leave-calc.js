...
// 勤続月数を日本語ラベルに変換（6→"6ヶ月", 18→"1年6ヶ月", 24→"2年"）
...
// 到達済みの最新付与マイルストーン（月数）を返す。テーブル最終行（66ヶ月）到達後は
// 12ヶ月ごとに延長し、毎年の更新付与も追跡する。初回付与（6ヶ月）未到達なら null。
function reachedMilestoneMonths(hire, now) {
  let reached = null;
  for (const t of LEAVE_GRANT_TABLE) {
    if (hasReachedGrantDate(hire, now, t.months)) reached = t.months;
  }
  if (reached === null) return null;
  const lastMonths = LEAVE_GRANT_TABLE[LEAVE_GRANT_TABLE.length - 1].months;
  if (reached === lastMonths) {
    let m = lastMonths + 12;
    while (hasReachedGrantDate(hire, now, m)) { reached = m; m += 12; }
  }
  return reached;
}

// マイルストーン月数に対応する付与日数（テーブル最終行以降は最終日数で頭打ち）
function grantDaysForMonths(months) {
  let days = 0;
  for (const t of LEAVE_GRANT_TABLE) {
    if (months >= t.months) days = t.days;
  }
  return days;
}

// 有給付与の時期が到来しているが、まだ反映されていないスタッフの情報を返す。
// 反映済み・対象外（入社日未設定・アーカイブ・初回付与未到達）なら null。
// 判定:
//   ・付与日数が増える区間(〜66ヶ月): 付与日数 or 付与履歴で反映済みを判定
//   ・20日で頭打ちの毎年更新区間(66ヶ月超): 付与日数では判別できないため付与履歴で判定
//     （付与履歴が無い既存スタッフは大量アラートを避けるため反映済み扱い＝初回記録以降を追跡）
function calcPendingGrant(staff, todayStr) {
  if (!staff || !staff.hire_date || staff.archived) return null;
  const today = todayStr || getTodayJST();
  const hire = new Date(staff.hire_date);
  const now  = new Date(today);

  const reached = reachedMilestoneMonths(hire, now);
  if (reached === null) return null;           // まだ初回付与（6ヶ月）に未到達
  const reachedDays = grantDaysForMonths(reached);
  const granted = staff.leave_granted || 0;

  const history = Array.isArray(staff.leave_grant_history) ? staff.leave_grant_history : [];
  const historyMax = history.reduce((mx, h) => Math.max(mx, h.months || 0), 0);
  const lastMonths = LEAVE_GRANT_TABLE[LEAVE_GRANT_TABLE.length - 1].months;

  let applied;
  if (reached <= lastMonths) {
    // 付与日数が増える区間: 付与日数 or 履歴で反映済みと判定できる
    applied = granted >= reachedDays || historyMax >= reached;
  } else {
    // 20日で頭打ちの毎年更新区間: 付与日数では判別できないので履歴で判定。
    // ただし履歴の無い既存スタッフが既に最大付与(20日)なら「反映済み」とみなし大量アラートを避ける。
    applied = historyMax >= reached || (history.length === 0 && granted >= reachedDays);
  }
  if (applied) return null;

  return {
    grant_days: reachedDays,
    granted,
    reached_months: reached,
    reached_date: toDateStr(addMonthsToDate(hire, reached)),
    tenure_label: formatTenureLabel(reached),
  };
}
...
// お祝い休暇の使用日数を計算（手動調整 + celebration申請 + 通常申請の部分消費）を
// 付与日数(celebration_days)を上限にキャップして返す。
// approvedRequests を渡すと loadLeave() を使わず直接計算（本人画面・サマリで共通利用）。
function calcCelebrationUsed(staff, approvedRequests) {
  let approved;
  ...
  let used = staff.celebration_used_adj || 0;
  for (const r of approved) {
    if (used >= celebrationDays) break;
    if (r.type === 'celebration') {
      ...
      used = Math.min(celebrationDays, used + r.celebration_days);
    }
  }
  return Math.round(Math.min(used, celebrationDays) * 10) / 10;
}
...
function calcLeaveBalance(staff, approvedRequests) {
  ...
  let usedDays = 0;
  for (const r of approved) {
    if (r.type === 'celebration') continue; // お祝い休暇は有給残日数から除外
    const perDate = (r.type === 'half_am' || r.type === 'half_pm') ? 0.5 : 1;
    const totalDays = r.dates.length * perDate;
    const celebPortion = r.celebration_days || 0; // 部分消費: お祝い休暇から消費した分
