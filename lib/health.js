...
async function runHealthChecks() {
  ...
  // 3. NDJSON 書き込み可能性
  try {
    if (!AUDIT_NDJSON_PATH) throw new Error('AUDIT_NDJSON_PATH が未定義（constants.js の再デプロイが必要）');
  ...
  // 6. Google Sheets API 接続チェック
  try {
    const { getValues } = require('./sheets');
    const { getSpreadsheetIdForYear } = require('./data');
    const sid = getSpreadsheetIdForYear(new Date().getFullYear());
    await getValues(sid, '1月!A1');
    checks.push({ name: 'Sheets API接続', ok: true, detail: 'OK' });
  } catch (e) {
    checks.push({ name: 'Sheets API接続', ok: false, detail: e.message });
  }
  ...
  try {
    ...
    const needsCols = activeStaff.filter(s => s.type !== 'office' && s.type !== 'admin');
    const missingCols = needsCols.filter(s =>
    ...
    checks.push({
      name: 'スタッフ列定義',
      ok,
      detail: ok ? `全${activeStaff.length}名設定済み` : `列未設定: ${missingCols.map(s => s.name).join(', ')}`,
    });
  } catch (e) {
    checks.push({ name: 'スタッフ列定義', ok: false, detail: e.message });
  }
