...
// 起動時：小澤悠那・大江綾子のお祝い休暇有効期限を1年に設定（一回限りのマイグレーション）
function migrateCelebrationExpiryTo1Year() {
  const targets = [
    { id: 'ozawa06' },
    { id: 'ooe07', hire_date: '2026-04-01' },
  ];
  const data = loadStaff();
  let changed = false;
  for (const t of targets) {
    const s = data.staff.find(st => st.id === t.id);
    if (!s) continue;
    if (s.celebration_expiry_months !== 12) {
      s.celebration_expiry_months = 12;
      changed = true;
      console.log(`✅ お祝い休暇有効期限を1年に設定: ${s.name} (${s.id})`);
    }
    if (t.hire_date && !s.hire_date) {
      s.hire_date = t.hire_date;
      changed = true;
      console.log(`✅ 入職日を設定: ${s.name} (${s.id}) = ${t.hire_date}`);
    }
  }
  if (changed) saveStaff(data);
}
