...
const { getNowJST } = require('./helpers');
const { DATA_DIR } = require('./constants');

// 起動時：未ハッシュPWをハッシュ化 & 旧 'rehab' 職種を 'PT' に移行 & 平文PW消去
...
function syncLeaveFieldsFromSource() {
  ...
  const liveData = loadStaff();
  let changed = false;
  const syncFields = ['hire_date','leave_granted','leave_grant_date','leave_carried_over','leave_manual_adjustment','oncall_eligible','oncall_leave_granted','celebration_days','celebration_used_adj','celebration_expiry_months'];
...
function ensureLeaveFields() {
  ...
  for (const s of data.staff) {
    ...
    if (s.oncall_leave_history === undefined)  { s.oncall_leave_history = [];     changed = true; }
    if (s.leave_grant_history === undefined)   { s.leave_grant_history = [];      changed = true; }
...
// 起動時：INITIAL_ADMIN_STAFF_IDS（カンマ区切り、複数）または INITIAL_ADMIN_STAFF_ID（単数、後方互換）
// に列挙された全スタッフを管理者にする。毎起動で idempotent に is_admin=true を union 追加する
// （既存の is_admin=true は減らさない）。UI で revoke したい場合は環境変数から除外すること。
function ensureInitialAdmins() {
  const idsEnv = process.env.INITIAL_ADMIN_STAFF_IDS || '';
  const idsNew = idsEnv.split(',').map(s => s.trim()).filter(Boolean);
  const idLegacy = (process.env.INITIAL_ADMIN_STAFF_ID || '').trim();
  const ids = [...new Set([...idsNew, ...(idLegacy ? [idLegacy] : [])])];

  const data = loadStaff();
  const hasAnyAdmin = data.staff.some(s => s.is_admin && !s.archived);

  if (ids.length === 0) {
    if (!hasAnyAdmin) {
      console.warn('⚠️ 管理者が未設定です。INITIAL_ADMIN_STAFF_IDS 環境変数を設定してください。');
    }
    return;
  }
  ...
  let changed = false;
  for (const id of ids) {
    const staff = data.staff.find(s => s.id === id && !s.archived);
    if (!staff) {
      console.warn(`⚠️ INITIAL_ADMIN_STAFF_IDS: ${id} に該当するスタッフが見つかりません`);
      continue;
    }
    if (!staff.is_admin) {
      staff.is_admin = true;
  ...
  if (changed) saveStaff(data);
}

// 起動時：INITIAL_ADMIN_PASSWORD が設定されている場合、INITIAL_ADMIN_STAFF_IDS の全スタッフのパスワードをリセット
// 使用後は必ず環境変数から削除すること
function applyInitialAdminPassword() {
  const pass = (process.env.INITIAL_ADMIN_PASSWORD || '').trim();
  const ids  = (process.env.INITIAL_ADMIN_STAFF_IDS || '').split(',').map(s => s.trim()).filter(Boolean);
  if (!pass || ids.length === 0) return;
  const data = loadStaff();
  let changed = false;
  for (const id of ids) {
    const staff = data.staff.find(s => s.id === id && !s.archived);
    if (!staff) { console.warn(`⚠️ INITIAL_ADMIN_PASSWORD: ${id} が見つかりません`); continue; }
    staff.password_hash = bcrypt.hashSync(pass, 10);
    changed = true;
    console.log(`✅ 管理者パスワードをリセットしました: ${staff.name} (${staff.id})`);
  }
  if (changed) saveStaff(data);
}
...
// 起動時：お祝い休暇有効期限マイグレーション
// - 小澤・大江: 12ヶ月（既存維持、初回のみ入職日設定）
// - 廣瀬・西口: 6ヶ月に変更
function migrateCelebrationExpiry() {
  const data = loadStaff();
  let changed = false;
  for (const s of data.staff) {
    // 小澤・大江: 12ヶ月維持（初回マイグレーション済みのため条件なし）
    if (s.id === 'ooe07' && !s.hire_date) {
      s.hire_date = '2026-04-01';
      changed = true;
      console.log(`✅ 入職日を設定: ${s.name} (${s.id}) = 2026-04-01`);
    }
    // 廣瀬・西口: 6ヶ月に設定
    if (s.name && (s.name.includes('廣瀬') || s.name.includes('西口'))) {
      if (s.celebration_expiry_months !== 6) {
        s.celebration_expiry_months = 6;
        changed = true;
        console.log(`✅ お祝い休暇有効期限を6ヶ月に設定: ${s.name} (${s.id})`);
      }
    }
  }
  if (changed) saveStaff(data);
