...
function loadStaff() {
  ...
  const raw = meta ? JSON.parse(meta.value) : {};
  const incentive_defaults = { nurse: raw.nurse ?? 3.5, rehab: raw.rehab ?? 20.0, nurse_rate: raw.nurse_rate, rehab_rate: raw.rehab_rate };
...
// ─── 訪問件数（SQLite正本化の移行用。当面はGoogle Sheetsと二重書き込み） ───
// 1件アップサート（看護師: {kaigo, iryo} / PT: {value}）
function upsertVisitRecord(staffId, date, { kaigo = null, iryo = null, value = null } = {}, updatedAt = null) {
  const db = getDb();
  db.prepare(
    'INSERT OR REPLACE INTO visit_records (staff_id, date, kaigo, iryo, value, updated_at) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(staffId, date, kaigo, iryo, value, updatedAt);
}

// 既存を上書きしない挿入（過去データ移行用）。既に(staff_id,date)があれば何もしない。
// → 二重書き込み済みの正しいデータをシートの古い値で上書きしてしまう事故を防ぐ。
function insertVisitRecordIfAbsent(staffId, date, { kaigo = null, iryo = null, value = null } = {}, updatedAt = null) {
  const db = getDb();
  const info = db.prepare(
    'INSERT OR IGNORE INTO visit_records (staff_id, date, kaigo, iryo, value, updated_at) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(staffId, date, kaigo, iryo, value, updatedAt);
  return info.changes > 0; // 実際に挿入したら true
}

// 1件取得（無ければ null）
function getVisitRecord(staffId, date) {
  const db = getDb();
  return db.prepare(
    'SELECT staff_id, date, kaigo, iryo, value, updated_at FROM visit_records WHERE staff_id = ? AND date = ?'
  ).get(staffId, date) || null;
}

// 訪問件数を getValues と同じ行配列形式でSQLiteから返す（段階4: 読み取り正本化）。
// 看護師: 各行 [kaigo, iryo] / PT: 各行 [value]。値は null→''、記録なしは空セル相当。
// startDay..endDay の各日について1行（getValuesのDATA_START_ROW起点の並びに一致）。
function getVisitSheetRows(staff, year, month, startDay, endDay) {
  const db = getDb();
  const mm = String(month).padStart(2, '0');
  const start = `${year}-${mm}-${String(startDay).padStart(2, '0')}`;
  const end   = `${year}-${mm}-${String(endDay).padStart(2, '0')}`;
  const recs = db.prepare(
    'SELECT date, kaigo, iryo, value FROM visit_records WHERE staff_id = ? AND date >= ? AND date <= ?'
  ).all(staff.id, start, end);
  const byDay = {};
  for (const r of recs) byDay[Number(r.date.slice(8, 10))] = r;
  const isNurse = staff.type === 'nurse';
  const rows = [];
  for (let day = startDay; day <= endDay; day++) {
    const r = byDay[day];
    if (isNurse) rows.push(r ? [r.kaigo == null ? '' : r.kaigo, r.iryo == null ? '' : r.iryo] : ['', '']);
    else rows.push(r ? [r.value == null ? '' : r.value] : ['']);
  }
  return rows;
}

// 指定日の各アクティブスタッフの入力状況（SQLite正本）。{missing, entered, onLeave}。
// isOnLeave(staffId, dateStr) は循環参照回避のため呼び出し側から渡す。
function getRecordStatusForDate(dateStr, isOnLeave) {
  const staffData = loadStaff();
  const active = staffData.staff.filter(s => !s.archived && (
  ...
  for (const s of active) {
    ...
    if (isOnLeave && isOnLeave(s.id, dateStr)) { results.onLeave.push(info); continue; }
    const rec = getVisitRecord(s.id, dateStr);
    const hasRecord = !!rec && (s.type === 'nurse' ? (rec.kaigo != null || rec.iryo != null) : rec.value != null);
    (hasRecord ? results.entered : results.missing).push(info);
  }
  return results;
}

// 期間取得（date昇順）
function getVisitRecordsRange(staffId, startDate, endDate) {
  const db = getDb();
  return db.prepare(
    'SELECT staff_id, date, kaigo, iryo, value, updated_at FROM visit_records WHERE staff_id = ? AND date >= ? AND date <= ? ORDER BY date'
...
module.exports = {
  atomicModify,
  upsertVisitRecord,
  insertVisitRecordIfAbsent,
  getVisitRecord,
  getVisitRecordsRange,
  getVisitSheetRows,
  getRecordStatusForDate,
  loadRegistry,
  saveRegistry,
