...
router.get('/api/monthly-stats', requireStaff, async (req, res) => {
  ...
  try {
    if (staff.type === 'nurse') {
      ...
      const expectedDays = countBusinessDays(yv.value, mv.value);
      ...
      const expectedDays2 = countBusinessDays(yv.value, mv.value);
  ...
  try {
    ...
    for (let i = 0; i < (daysInPrev - (BILLING_DAY - 1)); i++) {
      ...
      if (isNurse) {
        ...
        days.push({ date: dateStr, day, month: prevM, weekday: WD[new Date(prevYear, prevM - 1, day).getDay()], kaigo, iryo, total });
      } else {
        const value = (row[0] !== undefined && row[0] !== '') ? parseFloat(row[0]) : null;
        if (value != null) { total_units += value; working_days++; }
        days.push({ date: dateStr, day, month: prevM, weekday: WD[new Date(prevYear, prevM - 1, day).getDay()], value });
      }
    }
    // 当月1日〜15日
    for (let i = 0; i < 15; i++) {
      ...
      if (isNurse) {
        ...
        if (kaigo != null || iryo != null) working_days++;
        const total = (kaigo != null || iryo != null) ? (kaigo || 0) + (iryo || 0) : null;
        days.push({ date: dateStr, day, month: m, weekday: WD[new Date(y, m - 1, day).getDay()], kaigo, iryo, total });
      } else {
        const value = (row[0] !== undefined && row[0] !== '') ? parseFloat(row[0]) : null;
        if (value != null) { total_units += value; working_days++; }
        days.push({ date: dateStr, day, month: m, weekday: WD[new Date(y, m - 1, day).getDay()], value });
      }
    }
    ...
    if (isNurse) {
      ...
      const incentive_amount = Math.floor(over_hours / 0.5) * 2000;
      ...
      res.json({ type: 'rehab', year: y, month: m, mode: 'billing',
        billing_start: billingStart, billing_end: billingEnd, days,
    ...
    console.error('billing-detail error:', e.message);
...
router.get('/api/monthly-detail', requireStaff, async (req, res) => {
  ...
  try {
    if (staff.type === 'nurse') {
      ...
      const expectedDaysN = countBusinessDays(y, m);
      ...
      const expectedDaysR = countBusinessDays(y, m);
